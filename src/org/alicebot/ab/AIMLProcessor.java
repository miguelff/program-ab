...
public class AIMLProcessor {
    ...
    private static void categoryProcessor(Node n, ArrayList<Category> categories, String topic, String aimlFile, String language) {
        ...
        for (int j = 0; j < children.getLength(); j++) {
            //ResourceProvider.Log.info("CHILD: "+children.item(j).getNodeName());
            ...
            else ResourceProvider.Log.info("categoryProcessor: unexpected "+mName);
        }
        ...
        if (language.equals("JP") || language.equals("jp")) {
            ...
            ResourceProvider.Log.info("<pattern>"+pattern+"</pattern> --> <pattern>"+morphPattern+"</pattern>");
            pattern = morphPattern;
            String morphThatPattern = JapaneseTokenizer.morphSentence(that);
            ResourceProvider.Log.info("<that>"+that+"</that> --> <that>"+morphThatPattern+"</that>");
            that = morphThatPattern;
            String morphTopicPattern = JapaneseTokenizer.morphSentence(topic);
            ResourceProvider.Log.info("<topic>"+topic+"</topic> --> <topic>"+morphTopicPattern+"</topic>");
            topic = morphTopicPattern;
        }
        Category c = new Category(0, pattern, that, topic, template, aimlFile);
        categories.add(c);
        //ResourceProvider.Log.info("CATEGORY: "+Category.categoryToIF(c));
        //ResourceProvider.Log.info("template="+Category.IFToCategory(Category.categoryToIF(c)).getTemplate());
    }
    ...
    public static ArrayList<Category> AIMLToCategories (String directory, String aimlFile) {
        try {
            ...
            for (int i = 0; i < nodelist.getLength(); i++)   {
                ...
                //ResourceProvider.Log.info("AIML child: "+n.getNodeName());
                if (n.getNodeName().equals("category")) {
                    ...
                    for (int j = 0; j < children.getLength(); j++) {
                        Node m = children.item(j);
                        //ResourceProvider.Log.info("Topic child: "+m.getNodeName());
            ...
            ResourceProvider.Log.info("AIMLToCategories: "+ex);
            ex.printStackTrace();
            return null;
        }
    }
 ...
 public static String respond(String input, String that, String topic, Chat chatSession, int srCnt) {
        ...
        response = MagicStrings.default_bot_response;
         try {
            ...
            //ResourceProvider.Log.info("Template="+leaf.category.getTemplate());
            ...
            //ResourceProvider.Log.info("That="+that);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        return response;
    ...
    public static String evalTagContent(Node node, ParseState ps, Set<String> ignoreAttributes) {
        ...
        for (int i = 0; i < childList.getLength(); i++) {
            ...
            ResourceProvider.Log.info("Something went wrong with evalTagContent");
            ex.printStackTrace();
        }
        return result;
    }
    ...
    private static String srai(Node node, ParseState ps) {
        ...
        try {
            ...
            if (MagicBooleans.trace_mode) {
                ResourceProvider.Log.info(trace_count+". <srai>"+result+"</srai> from "+ps.leaf.category.inputThatTopic()+" topic="+topic+") ");
                trace_count++;
            }
            Nodemapper leaf = ps.chatSession.bot.brain.match(result, ps.that, topic);
            if (leaf == null) {return(response);}
            //ResourceProvider.Log.info("Srai returned "+leaf.category.inputThatTopic()+":"+leaf.category.getTemplate());
    ...
    private static String getAttributeOrTagValue (Node node, ParseState ps, String attributeName) {        // AIML 2.0
        //ResourceProvider.Log.info("getAttributeOrTagValue "+attributeName);
        String result = "";
        ...
        if (m == null) {
            ...
            for (int i = 0; i < childList.getLength(); i++)   {
                Node child = childList.item(i);
                //ResourceProvider.Log.info("getAttributeOrTagValue child = "+child.getNodeName());
                if (child.getNodeName().equals(attributeName)) {
                    result = evalTagContent(child, ps, null);
                    //ResourceProvider.Log.info("getAttributeOrTagValue result from child = "+result);
                }
            }
        }
        else {
        ...
        //ResourceProvider.Log.info("getAttributeOrTagValue "+attributeName+" = "+result);
        return result;
    }
    ...
    private static String map(Node node, ParseState ps) {
        ...
        if (mapName == null) result = "<map>"+contents+"</map>"; // this is an OOB map tag (no attribute)
            ...
            //ResourceProvider.Log.info("AIMLProcessor map "+contents+" "+result);
    ...
    private static String bot(Node node, ParseState ps) {
        ...
        //ResourceProvider.Log.info("BOT: "+m.getNodeValue()+"="+result);
        return result;
    }
    ...
    private static String that(Node node, ParseState ps) {
        ...
        if (value != null)
            try {
                ...
                ResourceProvider.Log.info("That index="+index+","+jndex);
    ...
    private static String learn(Node node, ParseState ps)   {                 // learn, learnf AIML 2.0
        ...
        for (int i = 0; i < childList.getLength(); i++) {
            if (childList.item(i).getNodeName().equals("category")) {
                ...
                //ResourceProvider.Log.info("Learn Pattern = "+pattern);
                ...
                if (MagicBooleans.trace_mode) {
                    ResourceProvider.Log.info("Learn Pattern = "+pattern);
                    ResourceProvider.Log.info("Learn That = "+that);
                    ResourceProvider.Log.info("Learn Template = "+template);
                }
                Category c;
    ...
    private static String condition(Node node, ParseState ps) {
        ...
        // otherwise this is a <condition> with <li> items:
            ...
            //ResourceProvider.Log.info("condition name="+liPredicate+" value="+value);
            if (value != null) {
    ...
    private static String recursEval(Node node, ParseState ps) {
        ...
        if (nodeName.equals("#text")) return node.getNodeValue();
            ...
            //ResourceProvider.Log.info("recursEval comment = "+node.getTextContent());
            return "";
        }
    ...
    public static boolean validTemplate(String template) {
        try {
            ...
            //e.printStackTrace();
            ResourceProvider.Log.info("Invalid Template "+template);
            return false;
        }
